...
async def explain_free_question(update: Update, context: ContextTypes.DEFAULT_TYPE):
    lang = get_lang(update)
    # Помечаем, что пользователь может задавать свободные вопросы
...
async def forward_free_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Не обрабатываем сообщения владельца и самого бота,
    # чтобы избежать зацикливания и лишнего шума.
...
".join([ln for ln in lines_out if ln != ""])
    ...
    # Сообщение пользователю о том, что вопрос передан
    await update.message.reply_text(
        t("free_q_user", lang),
        reply_markup=main_menu_keyboard(lang, free_mode=True),
    )

    # Дополнительно предлагаем оставить контакт через inline-кнопки
...
async def free_contact_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обработка inline-кнопок после свободного вопроса (телефон / username)."""
    ...
    if data == "free_contact_phone":
        # Отправляем reply-клавиатуру с запросом номера телефона
    ...
    if data == "free_contact_username":
        ...
        # Отправляем контакт по username владельцу
        if OWNER_CHAT_ID:
            lines = [
...
(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # здесь их игнорируем, чтобы не мешать основной логике меню.
    ...
    # По умолчанию — стандартное сообщение о неизвестной команде
    await update.message.reply_text(
...
async def plan_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == PLAN_WHAT:
        ...
        text = (
            ...
            "1. Врач-генетик объяснит, о каком заболевании идёт речь, как оно проявляется.\n"
...
async def owner_auto_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Автоответ работает только при ответе на сообщение бота
